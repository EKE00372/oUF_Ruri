...
do
	...
	local hideRaidFrames = hideNative and F.GetRuriOption("RaidFrames")
	local hidePartyFrames = hideNative and F.GetRuriOption("PartyFrames")

	if hideRaidFrames or hidePartyFrames then
		-- 隱藏原生團隊框架，但保留團隊管理介面
		local function HideRaidFrames()
			...
			if GetCVar("raidOptionIsShown") ~= "0" then
		...
		local function ApplyFrames()
			if hideRaidFrames then HideRaidFrames() end
			if hidePartyFrames then HidePartyFrames() end
		end
