...
local function Update(self, event, ...)
	...
	if eventFilter[event] then
		...
		local isPet = C.PetCombatText and DB:IsMyPet(sourceFlags)
		...
		if isRightUnit and (unit == "target" and (isPlayer or isPet) or unit == "player") then
			...
			if value.suffix == "DAMAGE" then
				if value.autoAttack and not C.AutoAttack then return end
				if value.isPeriod and not C.HotsDots then return end
			...
			elseif value.suffix == "HEAL" then
				...
				if amount == 0 and not C.FCTOverHealing then return end
	...
	if text and texture then
		...
		if C.ScrollingCT then
			element.Scrolling:AddMessage(format(element.format, texture, F.Hex(color)..(critMark and "*" or "")..text))
		else
			local string = getAvailableString(element)

			string:SetFont(element.font, C.FCTFontSize * multiplier, element.fontFlags)
